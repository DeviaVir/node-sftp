...
(function() {
    ...
    this.connect = function(cbconnect) {
        if (!(this.state & Sftp.STATE_DISCONNECTED)) {
            if (cbconnect) {
                cbconnect( this.state
                    & Sftp.STATE_CONNECTED ? null : this.state
                        & Sftp.STATE_CONNECTING ? "SFTP Error: already connecting to a host, please be patient" : "SFTP Error: invalid state."
                );
            }
            return;
        }
        ...
        this.socket.on("data", function(data) {
            ...
            if(o.password && passwordSent === 0) {
                //console.log('requires password, sending to socket in response to data');
                _self.socket.write(o.password + '\r');
                passwordSent = 1;
            }
    ...
    this.disconnect = function(cbdisconn) {
        ...
        this.exec("bye", "bye", function(lines) {
            ...
            if(_self.child && _self.child.kill)
                _self.child.kill();
            _self.child = null;
            if(_self.socket)
                _self.socket.destroy();
    ...
    function parseReply(data) {
        ...
        if (data.indexOf("sftp>") > -1 || (this.activeCmd == "bye" && data.indexOf("bye") > -1)) {
            ...
            else if (this.callbacks[this.activeCmd]) {
                cbdone = this.callbacks[this.activeCmd];
        ...
        else if (this.callbacks[this.activeCmd + "_progress"]) {
            cbprogress = this.callbacks[this.activeCmd + "_progress"];
